...
class prosesCrud {
    ...
    function tampil_data_id_demi($tabel,$where,$id_demi)
    {
        $row = $this->db->prepare("SELECT * FROM $tabel WHERE $where = ?");
        $row->execute(array($id_demi));
        return $hasil = $row->fetch();
    }
    ...
    function tambah_data_demi($tabel,$data)
    {
        // buat array untuk isi values insert sumber kode 
        // http://thisinterestsme.com/pdo-prepared-multi-inserts/
        $rowsSQL = array();
...
function edit_data_demi($tabel,$data,$where,$id_demi)
    {
        // sumber kode 
        // https://stackoverflow.com/questions/23019219/creating-generic-update-function-using-php-mysql
        $setPart = array();
        ...
        $sql = "UPDATE $tabel SET ".implode(', ', $setPart)." WHERE $where = :id_demi";
        $row = $this->db->prepare($sql);
        //Bind our values.
        $row ->bindValue(':id_demi',$id_demi); // where
        foreach($data as $param => $val)
        {
            $row ->bindValue($param, $val);
        }
    ...
    function hapus_demi($tabel,$where,$id_demi)
    {
        $sql = "DELETE FROM $tabel WHERE $where = ?";
        $row = $this->db->prepare($sql);
        return $row ->execute(array($id_demi));
    }
